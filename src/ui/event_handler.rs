...
pub fn ui_event_processor(event_type: crate::exports::astrobox::psys_plugin::event::Event, event_id: &str, event_payload: &str) {
    if !matches!(
        event_type,
        crate::exports::astrobox::psys_plugin::event::Event::MouseEnter
            | crate::exports::astrobox::psys_plugin::event::Event::MouseLeave
    ) {
        tracing::info!(
            "UI Event: type={:?}, id={}, payload={}",
            event_type,
            event_id,
            event_payload
        );
    ...
    if matches!(
        ...
        let should_rerender = {
            ...
            if state.hovered_id.as_deref() != Some(event_id) {
                state.hovered_id = Some(event_id.to_string());
                true
            } else {
                false
            }
